...
const { closeSideBarState, theme } = defineProps(['closeSideBarState']);
...
watch([closeSideBarState], (newVal, oldVal) => {
...
<template>
    ...
    <div class="fixed right-0 top-0 sm:w-[50%] w-[85%] h-screen overflow-y-auto transition-transform duration-500 ease-linear lg:hidden bg-white dark:bg-dark"
        :class="[closeSideBarState ? 'translate-x-0' : 'translate-x-[1000px]']">
        <div class="pt-5 p-5">
            <div class="flex justify-between items-center">
                <figure>
                    <img src="~/assets/img/zihux_logo.svg" class="w-[65px] object-contain" alt="logo">
                </figure>
            ...
            <div class="w-full flex gap-3  items-center my-5">
                <CommonUtilsRegisterBtn href="/user" class="bg-transparent w-1/2 text-center  font-medium border 
                    text-black border-gray-20 dark:text-white dark:border-gray-50/20">Dashboard
                </CommonUtilsRegisterBtn>
          <!--      <CommonUtilsRegisterBtn href="/register"
                    class="w-1/2 flex justify-center gap-1 items-center px-[.3rem] border border-dark dark:border-none">
                    <span>
            ...
            <div class="w-full h-[.1px] my-5 bg-dark/20' dark:bg-white/20"></div>

            <ul class="space-y-2">
                <li v-for="(data, key) in sidebarMore" :key="key" class="p-2">
                    <div class="">
                        <a :href="data.href" class="" v-if="!data.caret">
                            <div class="flex items-center justify-between text-dark dark:text-white">
