...
if (container.value) {
  const scrollAmount = container.value.clientWidth;
  const containerWidth = container.value.scrollWidth - container.value.clientWidth;
  const currentPosition = container.value.scrollLeft;

  if (direction === "left") {
    container.value.scrollLeft = Math.max(currentPosition - scrollAmount, 0);
  } else if (direction === "right") {
    container.value.scrollLeft = Math.min(currentPosition + scrollAmount, containerWidth);
  }
}
}
</script>
